...
def tratamiento_PI(sysCaso,kP,kI):
    num = [kP, kI]
    den = [1, 0]
...
sysHipertenso = tratamiento_PI(sysHiper,kP,kI)
...
plt.show()
fig2.savefig("hipertenso.pdf",bbox_inches = "tight")
#hipo
sysHipotenso = tratamiento_PI(sysHipo,kP,kI)
#normo
fig3 = plt.figure()
_, respN = ctrl.forced_response(sysN, t, u, x0)
plt.plot(t, respN, '-', color=Azul, label='Normotenso')
...
plt.title('HIPOTENSO')
plt.show()
fig3.savefig("hipotenso.pdf",bbox_inches = "tight")